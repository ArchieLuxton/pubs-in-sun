...
def crop_tiff(openpath, savepath, top, bottom, left, right):
    # Open the GeoTiff using Rasterio
    with rasterio.open(openpath) as src:
        ...
        # Create a new profile for the cropped GeoTiff
        profile = src.profile
        profile.update({
...
def geqc(num1, num2):
    """ This does num1>=num2, but also checks to see if the numbers are very similar"""
    if isclose(num1, num2, abs_tol=1e-6):
        return True
    elif num1>num2:
        return True
    else:
...
def lon_to_m(lon, lat):
    return lon*111320*np.cos(lat)

def latlon_to_m(lat, lon):
    return np.array([lat_to_m(lat), lon_to_m(lon, lat)])

def m_to_lat(m):
    return m/111320

def m_to_lon(m, lat):
    return m/(111320*np.cos(lat))


#def calculate_bounding_box(f):
#    minx = np.min(f[:,0])
#    maxx = np.max(f[:,0])
#    
#    miny = np.min(f[:,1])
...
def calculate_bounding_box(f):
    return np.stack((np.min(f, axis=0), np.max(f, axis=0)), axis=-1)

def calculate_bounding_box(f):
    return np.min(f, axis=0), np.max(f, axis=0)


def interp_df(df, xsplits=100, ysplits=100):
    # Convert the dataframe to a numpy array
    points = df[["lat", "lon"]].to_numpy()
    values = df["hours"].to_numpy()
    
    # Define the target grid for the interpolated values
    xi = np.linspace(df["lat"].min(), df["lat"].max(), num=xsplits)
    yi = np.linspace(df["lon"].min(), df["lon"].max(), num=ysplits)
    xi, yi = np.meshgrid(xi, yi)
    
    # Interpolate the values
    zi = griddata(points, values, (xi, yi), method='linear')
    
    # Flatten the arrays xi and yi into 1D arrays
    lat_interp = xi.flatten()
    lon_interp = yi.flatten()
    
    # Flatten the 2D array zi into a 1D array
    height_interp = zi.flatten()
    
    # Combine the arrays into a DataFrame
    df_interp = pd.DataFrame({"lat": lat_interp, "lon": lon_interp, "hours": height_interp})
    
    df_interp.dropna(inplace=True)
    return df_interp


### EVERYTHING BELOW FROM https://bitbucket.org/william_rusnack/minimumboundingbox/src/master/MinimumBoundingBox.py
